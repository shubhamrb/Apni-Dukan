...
import com.mponline.vendorApp.ui.adapter.InboxAdapter
...
class InboxFragment : BaseFragment(), OnItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        //Stores
        view?.rv_inbox?.setHasFixedSize(true)
        view?.rv_inbox?.layoutManager =
            LinearLayoutManager(
                activity,
                RecyclerView.VERTICAL,
                false
            )
        view?.rv_inbox?.adapter = InboxAdapter(
            activity,
            this
        )
