        ...
        if (CommonUtils.isOnline(activity!!)) {
            ...
            viewModel?.updateProfile(commonRequestObj)?.observe(this, Observer {
                it?.run {
                    switchView(1, "")
                    if(status){
                        mPreferenceUtils?.setValue(Constants.USER_NAME, edt_username.text.toString().trim())
                        mPreferenceUtils?.setValue(Constants.USER_EMAIL, edt_email.text.toString().trim())
                    }
                    CommonUtils.createSnackBar(
                        activity?.findViewById(android.R.id.content)!!,
                        message
