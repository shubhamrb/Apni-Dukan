 ...
 */
package com.mamits.apnaonlines.user.ui.base
...
import com.mamits.apnaonlines.user.util.CommonUtils.Companion.printLog
import dagger.hilt.android.AndroidEntryPoint
import java.text.DateFormat
import java.util.*

@AndroidEntryPoint
open class FusedLocationActivity : BaseActivity() {
    /**
        ...
        mLocationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult) {
                ...
                if (locationResult != null) {
                    ...
                    printLog("FetchedCurrLocation", Gson().toJson(mCurrentLocation))
                    onLocationSuccess()
                }
            }
        }
            ...
            .addOnFailureListener(this) { e ->
                ...
                when (statusCode) {
                    ...
                    LocationSettingsStatusCodes.SETTINGS_CHANGE_UNAVAILABLE -> {
                        ...
                        printLog(TAG, errorMessage)
                        Toast.makeText(this@FusedLocationActivity, errorMessage, Toast.LENGTH_LONG)
                            .show()
                    }
                }
            }
...
//                        });
    ...
    //                        val address =
    //                                this.get(0)?.getAddressLine(0) // If any additional address line present than only, check with max available address lines by getMaxAddressLineIndex()
    //                        val city = this.get(0)?.locality
    //                        val state = this.get(0)?.adminArea
    //                        val country = this.get(0)?.countryName
    //                        val postalCode = this?.get(0)?.postalCode
    //                        val knownName = this.get(0)?.featureName
    //                                userCurrentAddress =
    //                                UserCurrentAddress(address, city, state, country, postalCode, knownName)
    //                    }
    //                }
    //            } catch (e: Exception) {
    //                e.printStackTrace()
