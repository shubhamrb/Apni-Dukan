...
//        views?.text_nav_username!!.text = userName;//mPreferenceUtils.getValue(Constants.USER_NAME)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view?.run {
            ...
            var letter = mPreferenceUtils?.getValue(Constants.USER_NAME)?.toCharArray()?.get(0)?.toString()+mPreferenceUtils?.getValue(
                Constants.USER_NAME)?.toCharArray()?.get(1)?.toString()
