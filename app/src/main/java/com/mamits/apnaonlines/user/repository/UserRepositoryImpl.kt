package com.mamits.apnaonlines.user.repository
...
import com.mamits.apnaonlines.user.model.request.*
import com.mamits.apnaonlines.user.model.response.*
import com.mamits.apnaonlines.user.util.CommonUtils
import com.mamits.apnaonlines.user.util.Constants.Companion.EXCEPTION_500
...
class UserRepositoryImpl @Inject constructor(val apiService: NetworkAPIService, val userDao: UserDao):
    ...
    override fun getOrderHistory(commonRequestObj: CommonRequestObj): MutableLiveData<OrderHistoryResponse> {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = apiService?.getOrderHistory(commonRequestObj?.headerInfo?.Authorization!!,commonRequestObj)
                if (response?.isSuccessful!!) {
                    data.postValue(response?.body())
                } else {
                    errorOnAPI.postValue("${response.message()}")
    ...
    override fun getNewOrders(commonRequestObj: CommonRequestObj): MutableLiveData<OrderHistoryResponse> {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = apiService?.getNewOrders(commonRequestObj?.headerInfo?.Authorization!!)
                if (response?.isSuccessful!!) {
                    data.postValue(response?.body())
                } else {
                    errorOnAPI.postValue("${response.message()}")
    ...
    override fun getNotificationList(commonRequestObj: CommonRequestObj): MutableLiveData<NotificationListResponse> {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = apiService?.getNotificationList(commonRequestObj?.headerInfo?.Authorization!!,commonRequestObj)
                if (response?.isSuccessful!!) {
                    data.postValue(response?.body())
                } else {
                    errorOnAPI.postValue("${response.message()}")
