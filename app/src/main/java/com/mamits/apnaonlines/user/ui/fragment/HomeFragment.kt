...
import com.mamits.apnaonlines.user.listener.OnLocationFetchListener
...
import com.mamits.apnaonlines.user.model.response.GetHomeDataResponse
import com.mamits.apnaonlines.user.ui.activity.LoginActivity
import com.mamits.apnaonlines.user.ui.adapter.BannerPagerAdapter
import com.mamits.apnaonlines.user.ui.adapter.ServicesAdapter
import com.mamits.apnaonlines.user.ui.adapter.StoresAdapter
import com.mamits.apnaonlines.user.ui.adapter.TopProductAdapter
...
class HomeFragment : BaseFragment(), OnItemClickListener, OnLocationFetchListener {
    ...
    private var nearStoresAdapter: StoresAdapter? = null
    ...
    private val SCROLLING_RUNNABLE: Runnable = object : Runnable {
        override fun run() {
            try {
                rv_top_exam_forms.smoothScrollBy(
    ...
    private fun callHomeApi() {
        if (LocationUtils.getCurrentLocation() == null) {
            ...
            if (CommonUtils.isOnline(requireContext())) {
                ...
                viewModel?.getHomeData(commonRequestObj)?.observe(viewLifecycleOwner, Observer {
                    it?.run {
                        if (status) {
                            switchView(1, "")
                            setDataToUI(this)
    ...
    fun setDataToUI(mGetHomeDataResponse: GetHomeDataResponse) {
        mGetHomeDataResponse?.let {
            mView?.run {
                ...
                var estimatedImgWidth = width - (CommonUtils.convertDpToPixel(
                    activity!!,
                    15
                ) + CommonUtils.convertDpToPixel(activity!!, 200))
...
//        indicator_startup.setViewPager(viewpager_startup)
                ...
                if (it?.data?.top_storelist != null) {
                    ...
                    storesAdapter!!.setList(it?.data?.top_storelist)
                }
                ...
                if (it?.data?.productlist != null) {
                    ...
                    if (it?.data?.productlist?.size!! > 3) {
                        rv_top_exam_forms.addOnScrollListener(object :
...
//                                    rv_top_exam_forms.setAdapter(null)
                ...
                if (it?.data?.storelist != null) {
                    ...
                    nearStoresAdapter = StoresAdapter(
                        activity,
                        this@HomeFragment
                    )
                    rv_nearbystores?.adapter = nearStoresAdapter
                    nearStoresAdapter!!.setList(it?.data?.storelist)
                }
