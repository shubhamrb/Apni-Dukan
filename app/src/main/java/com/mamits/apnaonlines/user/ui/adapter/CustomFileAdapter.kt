...
import com.mamits.apnaonlines.user.ui.activity.MainActivity
...
class CustomFileAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is FilePickerViewHolder) {
            ...
            holder.itemView.text_file_upload_label.text =  if (mList?.get(position)?.isRequired?.equals("Yes")) Html.fromHtml(mList?.get(position)?.label+ "*") else Html.fromHtml(mList?.get(position)?.label)
            ...
            holder.itemView.text_edt_upload_label.text = if (mList?.get(position)?.isRequired?.equals("Yes")) Html.fromHtml(mList?.get(position)?.label+ "*") else Html.fromHtml(mList?.get(position)?.label)
...
//                holder.itemView.edt_custom_field_mult.inputType = InputType.TYPE_CLASS_NUMBER
            ...
            if (!mList?.get(position)?.value?.isNullOrEmpty()!!) {
                holder.itemView.text_spn_title.text = if (mList?.get(position)?.isRequired?.equals("Yes")) Html.fromHtml(mList?.get(position)?.label+ "*") else Html.fromHtml(mList?.get(position)?.label)
                //if (!isAllowedForListener) {
                holder.bind(context, mList?.get(position), listener)
                //}
            }
        }
            ...
            holder.itemView.text_chkbox_title.text = if (mList?.get(position)?.isRequired?.equals("Yes")) Html.fromHtml(mList?.get(position)?.label+ "*") else Html.fromHtml(mList?.get(position)?.label)
            ...
            holder.itemView.ll_radio_checkbox_container.visibility = if (mList?.get(position)?.isVisible) View.VISIBLE else View.GONE
            ...
            holder.itemView.ll_btn_container.visibility =
                if (mList?.get(position)?.isVisible) View.VISIBLE else View.GONE
            holder.itemView.text_btn_label.text = if (mList?.get(position)?.isRequired?.equals("Yes")) Html.fromHtml(mList?.get(position)?.label+ "*") else Html.fromHtml(mList?.get(position)?.label)
            holder.itemView.btn.setText(
                if (!mList?.get(position)?.ansValue?.isNullOrEmpty()!!) mList?.get(position)?.ansValue else Html.fromHtml(mList?.get(position)?.name)
            )
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        } else if (/*(mList?.get(position)?.id?.equals("location", true)!!) || */mList?.get(position)?.fieldType?.equals(
                "location",
                true
            )!! || mList?.get(
                position
    ...
    fun onRefreshAdapter(list: ArrayList<CustomFieldObj>, flag: Boolean) {
        mList = list
...
//                        if(!isAllowedForListener) {
                    CommonUtils.printLog(
                        "ANS_TXT_LISTNER_MULT ${adapterPosition}",
                        "${s.toString()}"
                    )
                    var customFieldObj = mList?.get(adapterPosition)
                    customFieldObj?.ansValue = s?.toString()
...
//                        }
    ...
    class DropdownViewHolder(var itemview: View) : RecyclerView.ViewHolder(itemview) {
        fun bind(
            ...
            if (!customFieldObj?.ansValue?.isEmpty()!!) {
                itemView.spn_text.text = customFieldObj?.ansValue
            } else {
                itemView.spn_text.text = "Select"
            }
            val builder = context?.let { AlertDialog.Builder(it) }

            val arrayAdapter =
                context?.let { ArrayAdapter<String>(it, android.R.layout.simple_spinner_dropdown_item) }

            if (dropdownItemList.size > 0) {
                dropdownItemList?.forEachIndexed { index, s ->
                    arrayAdapter?.add(s.value)

                }
            }

            builder?.setAdapter(arrayAdapter) { dialog, pos ->
                listener?.onClick(
                    adapterPosition,
                    itemView.spn_text,
            ...
            itemview.spn_text.setOnClickListener {
                dialog?.show()
            }
