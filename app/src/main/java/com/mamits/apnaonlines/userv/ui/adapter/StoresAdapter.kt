...
class StoresAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (!mList?.get(position)?.storelogo?.isNullOrEmpty()) {
            ImageGlideUtils.loadUrlImage(
                context!!,
                mList?.get(position)?.storelogo,
                holder.itemView.image_store
            )
        ...
        holder.itemView.text_store_loc.text = "${
            if (mList?.get(position)?.distance!! % 1 == 0.0) mList?.get(position)?.distance else mList?.get(
                position
            )?.distance
        }KM away from you"
        holder.itemView.ratingbar_store.rating =
            if (mList?.get(position)?.ratting != null && !mList?.get(position)?.ratting?.isNullOrEmpty() && !mList?.get(
                    position
                )?.ratting?.equals("zero")
            ) mList?.get(position)?.ratting?.toFloat() else 0f
    ...
    public fun setList(list: ArrayList<StorelistItem>) {
        mList = list
